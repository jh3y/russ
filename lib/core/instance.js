...
var BoltInstance = function () {
  ...
  _createClass(BoltInstance, [{
    ...
    value: function runTask(name) {
      ...
      if (!task.concurrent) {
        var _ret = function () {
          var taskPool = _this3.getPool(name);
          console.log(taskPool);
          return {
            v: new Promise(function (resolve, reject) {
              if (task.sequence) winston.info('Running ' + task.name);
              if (task.sequence || taskPool.length > 1) winston.profile('SOMETHING');
        ...
        if ((typeof _ret === 'undefined' ? 'undefined' : _typeof(_ret)) === "object") return _ret.v;
          ...
          winston.profile(name);
          var concurrentTasks = task.concurrent.map(this.runTask.bind(this));
          Promise.all(concurrentTasks).then(function () {
            winston.profile(name);
          });
        }
    }
