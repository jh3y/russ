...
var RussInstance = function () {
  function RussInstance(env) {
    _classCallCheck(this, RussInstance);
    ...
    if (!tasks || tasks.length == 0) tasks = fs.readdirSync('tasks');
    this.env = env;
    this.config = config;
    this.register(tasks);
    return this;
      ...
      var exec = function exec(name, resolve, reject) {
        ...
        try {
          ...
          if (taskObj.concurrent) {
            ...
            var task = void 0;
            try {
              task = new RussTask(_this, _this.tasks[name]);
            } catch (err) {
    ...
    value: function getPool(name) {
      ...
      var pushToPool = function pushToPool(name, parent) {
        ...
        if (parent) {
          ...
          var idx = name === parentTask.pre ? pIdx : pIdx + 1;
    ...
    */
    ...
    value: function register(files) {
      ...
      var registerTask = function registerTask(opts) {
        var ERR_MSG = 'Task ' + opts.name + ' missing properties...';
      ...
      try {
        for (var _iterator2 = files[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          var taskOpts = require(process.cwd() + '/russ.tasks/' + file);
