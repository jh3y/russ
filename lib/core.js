...
var BoltInstance = function () {
  ...
  _createClass(BoltInstance, [{
    ...
    value: function runTask(name, env) {
      if (this.tasks[name]) this.tasks[name].run(env).then(function () {
        winston.info('finished ' + name);
      }).catch(function (err) {
        winston.error('Error: ' + err);
      });else throw Error('no such task ' + name);
    }
  }, {
    key: 'info',
    value: function info() {
    ...
    value: function run(env) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_this.func && typeof _this.func === 'function')
          ...
          _this.func.apply(_this, _toConsumableArray(_this.deps).concat([env, _this.parent.config, resolve, reject]));
      });
    }
  }]);
